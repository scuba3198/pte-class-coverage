...
describe("State Merging Logic (Guest to Cloud)", () => {
  it("should detect if a state has meaningful data", () => {
    ...
    const hasData = (state: AppState) => {
      const hasCoverage = Object.values(state.coverage).some((c) =>
  ...
  it("identifies the 'local merge' condition correctly", () => {
    ...
    const cloudHasData = Object.values(cloudState.coverage).some((c) =>
      Object.values(c).some((v) => v === true),
    );
    const localHasData = Object.values(localState.coverage).some((c) =>
      Object.values(c).some((v) => v === true),
    );
