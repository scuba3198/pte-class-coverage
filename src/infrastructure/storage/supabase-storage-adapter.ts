...
export class SupabaseStorageAdapter implements StorageAdapter {
    ...
    async loadState(key: string): Promise<Result<AppState, StorageError>> {
        try {
            ...
            if (error) {
                ...
                this.logger.error("Failed to fetch state from Supabase", { error: error.message, code: error.code });
    ...
    async saveState(_key: string, state: AppState): Promise<Result<void, StorageError>> {
        try {
            ...
            const tickCount = Object.values(state.coverage).reduce((acc, c) =>
                acc + Object.values(c).filter(v => v === true).length, 0);
