...
export const classDefaults: readonly ClassItem[] = [
...
const questionTypeIdToModuleId = new Map<string, string>(
...
const weightageEntries: ResolvedWeightageEntry[] = weightageChart
    .map((entry) => ({
        ...entry,
    ...
    .filter(
        (entry): entry is ResolvedWeightageEntry =>
            !!entry.questionTypeId && !!entry.originModuleId
    );
...
const normalizeClasses = (classes: unknown): ClassItem[] => {
...
const normalizeSessionModuleId = (moduleId: string, questionTypeIds: string[]): string => {
    ...
    normalizedClasses.forEach((classItem) => {
        ...
        sessions[classItem.id] = existingSessions
            .map((session: unknown) => {
                ...
                const questionTypeIds = rawQuestionTypeIds.filter((id): id is string =>
                    typeof id === 'string' && allQuestionTypeIds.includes(id)
                );
...
export const mergeStates = (remoteState: unknown, localState: unknown): AppState => {
...
const getTopEntriesForSkillTarget = (skill: SkillKey, target = 72): ResolvedWeightageEntry[] => {
...
export const getCoverageEntriesForSkill = (skill: SkillKey, target = 72): ResolvedWeightageEntry[] =>
